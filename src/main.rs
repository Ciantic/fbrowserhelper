                    ...
                    if "MozillaDialogClass" == class_name {
                        std::thread::spawn(move || {
                            ...
                            if let Some(url) = get_url_from_string(&name) {
                                ...
                                let style = GetWindowLongA(target_window, GWL_STYLE);
                                SetWindowLongA(
                                    target_window,
                                    GWL_STYLE,
...
fn get_favicon_from_url(url: &Url) -> Result<String, GetFaviconError> {
    ...
    let icon = reqwest::blocking::get(format!("https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url={}&size=64", url))?;
